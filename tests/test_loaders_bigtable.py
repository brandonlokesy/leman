"""`BigTableSpectralSweep` — the headerless export, and what it refuses.
...
caller → `__init__` → `_decode_and_describe` → `BigTableSpectralSweep._decode`
...
@pytest.fixture
def ez():
    return BigTableSpectralSweep(EZ, spectra_type="PL", gates=GATES)
...
def test_shape_and_range(path, n_sweeps):
    scan = BigTableSpectralSweep(path, spectra_type="PL")
...
def test_the_positional_row_map_against_rows_read_by_hand():
    ...
    scan = BigTableSpectralSweep(EZ, spectra_type="PL")
    p = scan.parameters
...
def test_gate_mode_recovers_what_the_filename_says(path, expected):
    ...
    scan = BigTableSpectralSweep(path, spectra_type="PL", gates=GATES)
...
def test_the_field_scan_resolves_a_field_axis():
    scan = BigTableSpectralSweep(EZ, spectra_type="PL", gates=GATES,
...
def test_the_doping_scan_resolves_a_density_axis():
    scan = BigTableSpectralSweep(DOPING, spectra_type="PL", gates=GATES,
...
def test_a_row_label_works_as_a_sweep_axis():
    ...
    scan = BigTableSpectralSweep(EZ, spectra_type="PL", sweep="Row 12",
...
def test_declaring_a_held_row_as_the_axis_warns():
    ...
    with pytest.warns(UserWarning, match="takes only 1 value"):
        BigTableSpectralSweep(EZ, spectra_type="PL", sweep="Row 17")


def test_the_correction_ladder_runs(ez):
    scan = BigTableSpectralSweep(
...
def test_a_headed_export_is_refused_and_the_right_class_named(path, names):
    ...
    with pytest.raises(ValueError, match=f"Use {names} instead"):
        BigTableSpectralSweep(path, spectra_type="PL")
...
def test_an_image_whose_width_does_not_divide_is_refused():
    ...
    with pytest.raises(ValueError, match="Use AttoCubeSpectralSweep instead"):
        BigTableSpectralSweep(path, spectra_type="PL")
...
def test_the_attocube_loader_refuses_a_bigtable_file():
    ...
    with pytest.raises(ValueError, match="BigTableSpectralSweep"):
        AttoCubeSpectralSweep(EZ, spectra_type="PL")
...
def test_there_is_no_roi_argument(tmp_path):
    ...
    with pytest.raises(TypeError, match="roi"):
        BigTableSpectralSweep(path, spectra_type="PL", roi=1)
